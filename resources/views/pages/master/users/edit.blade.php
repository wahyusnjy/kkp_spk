    <x-layouts.app :title="__('Edit users')">
        <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-white">Edit Users {{ $user->name }}</h1>
                    <p class="text-gray-400 mt-1">Edit User</p>
                </div>
                    ...
                    <div class="bg-dark-300 rounded-xl border border-dark-200 p-6">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-gray-400 text-sm">Isi form berikut untuk mengubah data users</p>
                            </div>
                        </div>
                        <form id="usersFormEdit" method="POST" action="{{ route('users.update', $user->id) }}">
                            @csrf
                            <div class="space-y-6">
                                <!-- Nama users -->
                                ...
                                <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                    ...
                                    <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                        ...
                                        <select id="role" name="role" required
                                            ...
                                            <option value="admin" {{ old('role')  ? (old('role') == 'admin' ? 'selected' : '') : ($user->role == 'admin' ? 'selected' : '') }}>Admin (Mengelola segala data dan perhitungan)</option>
                                            <option value="manager" {{ old('role') ? (old('role') == 'manager' ? 'selected' : '') : ($user->role == 'manager' ? 'selected' : '') }}>Manager (Cuma Bisa liat hasil perhitungan)</option>
                                        </select>
                                        @error('role')
                                            <div class="flex items-center gap-2 mt-2 text-red-400 text-sm">
                                                <i class="fas fa-exclamation-circle"></i>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const form = document.getElementById('usersFormEdit');
