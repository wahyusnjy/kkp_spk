<x-layouts.app :title="__('Tambah Supplier')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 rounded-xl">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white">Ubah Supplier {{ $supplier->nama_supplier }}</h1>
                <p class="text-gray-400 mt-1">Proses modifikasi data supplier {{ $supplier->nama_supplier }} ke sistem</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2">
                <div class="bg-dark-300 rounded-xl border border-dark-200 p-6">
                    ...
                    <form id="supplierFormCreate" method="POST" action="{{ route('supplier.update',$supplier->id) }}">
                        @csrf
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                    ...
                                    <input type="text" id="kode_supplier" name="kode_supplier" readonly required
                                        ...
                                        value="{{ old('kode_supplier') ? old('kode_supplier') : $supplier->kode_supplier }}">
                                    @error('kode_supplier')
                                        <div class="flex items-center gap-2 mt-2 text-red-400 text-sm">
                                            <i class="fas fa-exclamation-circle"></i>
                                            <span>{{ $message }}</span>
                                ...
                                <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                    ...
                                    <input type="text" id="kontak" name="kontak"
                                        ...
                                        value="{{ old('kontak') ? old('kontak') : $supplier->kontak }}">
                                    @error('kontak')
                                        <div class="flex items-center gap-2 mt-2 text-red-400 text-sm">
                                            <i class="fas fa-exclamation-circle"></i>
                                            <span>{{ $message }}</span>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                    ...
                                    <input type="text" id="kategori_material" name="kategori_material" required
                                        ...
                                        value="{{ old('kategori_material') ? old('kategori_material') : $supplier->kategori_material }}">
                                ...
                                <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                    ...
                                    <select id="status" name="status" required
                                        ...
                                        <option value="aktif" {{ old('status') ? old('status') == 'aktif' ? 'selected' : $supplier->status : 'selected' }}>Aktif</option>
                                        <option value="nonaktif" {{ old('status') ? old('status') == 'nonaktif' ? 'selected' : $supplier->status : 'selected' }}>Nonaktif</option>
                                    </select>
                                    @error('status')
                                        <div class="flex items-center gap-2 mt-2 text-red-400 text-sm">
                                            <i class="fas fa-exclamation-circle"></i>
                            ...
                            <div class="bg-dark-400/50 rounded-lg p-4 border border-dark-200">
                                ...
                                <textarea id="alamat" name="alamat" rows="3" required
                                        ...
                                        placeholder="Masukkan alamat lengkap supplier...">{{ old('alamat') ? old('alamat') : $supplier->alamat }}</textarea>
                                @error('alamat')
                                    <div class="flex items-center gap-2 mt-2 text-red-400 text-sm">
                                        <i class="fas fa-exclamation-circle"></i>
                                        <span>{{ $message }}</span>
